...
# Función para dibujar el HUD (Score | Coins)
# Función para dibujar el HUD (Score y Coins) - COMPACTO IZQUIERDA
draw_hud:
    addi $sp, $sp, -8
    sw $ra, 0($sp)
    sw $s0, 4($sp)
    ...
    li $a1, 2                   # Y
    lw $a2, score
    jal draw_number_compact     # Usaremos versión compacta
    ...
    li $a0, 36
    li $a1, 2
    jal draw_coin_sprite_small  # Versión 4x4
...
# ============= NUEVAS FUNCIONES PARA HUD COMPACTO =============

# Función mejorada: dibujar número con sprites 3x5 (más pequeños y limpios)
# $a0 = X position, $a1 = Y position, $a2 = number
draw_number_compact:
    addi $sp, $sp, -20
    sw $ra, 0($sp)
    sw $s0, 4($sp)
    ...
    # Convertir número a dígitos (máx 5)
    li $s3, 10000               # Divisor
    
draw_digit_compact_loop:
    beqz $s3, draw_number_compact_done
    ...
    addi $s0, $s0, 4            # Siguiente posición X (3 + 1 espacio)
    
    li $t1, 10
    div $s3, $t1
    mflo $s3                    # Siguiente divisor
    
    j draw_digit_compact_loop
...
# Dibujar dígito individual 3x5 (muy definido)
...
draw_digit_3x5:
    ...
    li $t9, 0xFFFFFF00          # Amarillo brillante
    
    # Switch para cada dígito
    beqz $a2, draw_3x5_0
    li $t0, 1
    beq $a2, $t0, draw_3x5_1
    li $t0, 2
    beq $a2, $t0, draw_3x5_2
    li $t0, 3
    beq $a2, $t0, draw_3x5_3
    li $t0, 4
    beq $a2, $t0, draw_3x5_4
    li $t0, 5
    beq $a2, $t0, draw_3x5_5
    li $t0, 6
    beq $a2, $t0, draw_3x5_6
    li $t0, 7
    beq $a2, $t0, draw_3x5_7
    li $t0, 8
    beq $a2, $t0, draw_3x5_8
    li $t0, 9
    beq $a2, $t0, draw_3x5_9
    j digit_3x5_done
...
# Sprite de moneda 4x4 (más pequeño para HUD)
draw_coin_sprite_small:
    addi $sp, $sp, -12
    sw $ra, 0($sp)
    sw $s0, 4($sp)
    sw $s1, 8($sp)
    ...
    # Dibujar moneda simple 4x4
    li $t0, 0xFFD700        # Dorado
